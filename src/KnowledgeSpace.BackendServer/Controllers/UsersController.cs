...
namespace KnowledgeSpace.BackendServer.Controllers
{
    public class UsersController : BaseController
    {
        ...
        public async Task<IActionResult> DeleteUser(string id)
        {
            ...
            var adminUser = await _userManager.GetUsersInRoleAsync(SystemConstants.Roles.Admin);
            var otherUser = adminUser.Where(x => x.Id != id).ToList();
            if(otherUser.Count == 0)
            {
                return BadRequest(new ApiBadRequestResponse("You cannot remove the only  amdin user  remaining!"));
            }
